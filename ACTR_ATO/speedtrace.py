...
# @Software: PyCharm
"""存在问题:
1. 时间过慢（速度从40降到0超过半个小时33分钟，还是最简单的情况）
2. 无法方正实际情况，界面闪动过快就追踪不上。
...
def button_pressed(len, dir):
    ...
    if not (choice):
        choice = dir
    if not (done):
        if int(speed_actual[0]) - int(speed_target[0]) > 0:
            speed_actual[0] = str(int(speed_actual[0]) - int(len))
        elif int(speed_actual[0]) - int(speed_target[0]) == 0:
...
if __name__ == '__main__':
    ...
    for i in range(2191,2379):
        ...
        speed_target[0] = round(SYL_spt2.train_model().target_v(i))
        print("运行次数： ",i)
    ...
    print("目标速度：",targetgroup)
    print("实际速度：", actualgroup)
