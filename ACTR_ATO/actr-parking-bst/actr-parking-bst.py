...
Assumption: a good driver can judge the approximate distance by looking at the speed and different brake levels
初始条件：初始距离，初始速度
Initial conditions: initial distance, initial velocity
这个程序的思路使让actr看到距离停车标的初始距离和实际到达精度30m，让actr从不同的制动级位的长度下进行选择，不断缩小距离最后达到实际精度
he purpose of the program enables actr to see the initial distance from the parking mark and the actual accuracy of 30m,
and allows actr to select from the length of different brake level positions to continuously reduce the distance and finally reach the actual accuracy
"""
...
def build_display(a, b, c, d, e, f, g, h, goal):
    "b is initial distance，c-a-d-e-f-g-h is different brake levels,goal is actual accuracy"
