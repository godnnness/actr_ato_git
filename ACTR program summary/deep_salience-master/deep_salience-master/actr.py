...
class interface():
    def __init__(self,host="192.168.100.129",port=2650):
...
def add_command(name,function,documentation="No documentation provided.",single=True,local_name=None):
...
def schedule_break_relative (time_delay,milliseconds=True, priority=0, details=None):
    return current_connection.evaluate_single("schedule-break-relative",time_delay,milliseconds,priority,details)
...
def schedule_simple_event_after_module (after_module, action, params=None, module='NONE', maintenance=False):
    return current_connection.evaluate_single("schedule-simple-event-after-module",after_module,action,params,module,maintenance)
...
def define_module (name, buffers,params,version,doc,interface):
    return current_connection.evaluate_single("define-module", name, buffers, params, version, doc, interface)
...
def create_image_for_exp_window(window,text,file,x=0,y=0,width=50,height=50,action=None):
    return current_connection.evaluate_single("create-image-for-exp-window",window,text,file,x,y,width,height,action)

def add_image_to_exp_window(window,text,file,x=0,y=0,width=50,height=50,action=None):
    return current_connection.evaluate_single("add-image-to-exp-window",window,text,file,x,y,width,height,action)
