...
if __name__ == "__main__":
    ...
    "Merciless":(1280,720), "Moonlight":(1920,808), "Mother":(1920,808), "Once":(720,390), "TokyoKazoku":(1024,560), "VIP":(1280,540), "ZeroDarkThirty":(1280,688) }

    argument = sys.argv[1]
    write_file_name = argument.split("/")[-1]
    # write_file_path = "./" + write_file_name + "/" + write_file_name + "_eyegaze_log_emmanoise.csv"
    ...
    stim_d = {key: {'text':key, 'position': (x[2], x[3])} for key,x in enumerate(sorted(list_of_obj[1802], key=lambda objs: objs[4],reverse=True))}
    print(stim_d)
