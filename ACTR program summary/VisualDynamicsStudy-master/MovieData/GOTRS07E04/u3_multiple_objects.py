...
if __name__ == "__main__":
    ...
    file_path = argument + "_2fps_Detected_objects.csv" # File name in the same folder (csv file to parse)
    ...
    for i in tqdm(range(len(list_of_obj))):
        ...
        stim_d = {key: {'text':x[0], 'position': (x[2], x[3])} for key,x in enumerate(sorted(list_of_obj[i], key=lambda objs: objs[4]))}
        print(stim_d)
    ...
    np.savetxt(argument + "_eyegaze_log.csv", gaze_data_all, delimiter=",")
    # log_file.close()